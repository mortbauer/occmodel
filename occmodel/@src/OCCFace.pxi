...
cdef class Face(Base):
    ...
    cpdef project_coords(self, coords, indices=None):
        import numpy as np
        ncoords = coords.copy()
        cdef c_OCCFace *occ = <c_OCCFace *>self.thisptr
        cdef double[:] coords_ = coords.ravel()
        cdef double[:] ncoords_ = ncoords.ravel()
        cdef int[:] indices_
        if indices is None:
            indices_ = np.arange(0,coords_.shape[0],dtype=np.int32)
        else:
            indices_ = indices
        cdef int n = indices.shape[0]
        cdef int ret = occ.project_array_of_coords(
            &coords_[0],&ncoords_[0],&indices_[0],n)
        if ret != 0:
            raise OCCError(errorMessage)
        return ncoords
